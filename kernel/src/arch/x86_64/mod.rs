...
fn map_physical_persistent_memory(desc_iter: impl Iterator<Item = &'static MemoryDescriptor>) {
    let kcb = kcb::get_kcb();
...
fn _start(argc: isize, _argv: *const *const u8) -> isize {
    ...
    // Initialize the machine topology (needs ACPI and alloc):
    {
        ...
        map_physical_persistent_memory(atopology::MACHINE_TOPOLOGY.persistent_memory());
